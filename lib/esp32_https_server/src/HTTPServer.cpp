...
uint8_t HTTPServer::setupSocket() {
  if (_bindAddress == 0) {
    ...
    if (_socket >= 0) {
      ...
      int err = setsockopt(_socket, IPPROTO_IPV6, IPV6_V6ONLY, &ipv6Only, sizeof(ipv6Only));
      if (!err) {
        err = bind(_socket, (struct sockaddr *)&socketAddress, sizeof(socketAddress));
      }
      if (!err) {
  ...
  // Fall back to an IPv4 listener if a dual-stack socket is unavailable.
