...
// Minimal self-contained SHA-1 (only used for the Websocket handshake).
// Kept local so this library does not depend on chip-specific hwcrypto headers
// (those are missing on ESP32-C3/S3, only present on the original ESP32).
...
std::string websocketKeyResponseHash(std::string const &key) {
  ...
  sha1((const uint8_t*)newKey.data(), newKey.length(), shaData);
